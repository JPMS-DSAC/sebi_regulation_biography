...
def ujwal_annotations(text):
	...
	for ent1 in doc1.ents:
        ...
        #Ask Ujwal to start the duckling service on a system and tell you its global/local ip for time tags.
        #Or ask sudo privileges from admin.
	url = "http://10.2.24.151:8000/parse"
	print(text)
...
def shravya_annotations(text):
	#Code to add ner, ..
	predicted = []
	nlp1 = spacy.load("objective_final/model-best") #load the best model
	nlp2 = spacy.load("authority_final/model-best") #load the best model
	nlp3 = spacy.load("subject-individual_final/model-best") #load the best model
	nlp4 = spacy.load("subject-organisation_final/model-best") #load the best model
	nlp5 = spacy.load("date_final/model-best") #load the best model
	nlp6 = spacy.load("legal_doc_final/model-best") #load the best model
	nlp7 = spacy.load("legal_term_final/model-best") #load the best model
	nlp8 = spacy.load("object_general_final/model-best") #load the best model
	nlp9 = spacy.load("transaction_final/model-best") #load the best model
	nlp10 = spacy.load("value_final/model-best") #load the best model
...
file1 = open("cleaned_rules.txt","r")
...
for text in all_samples:
		if text:
				...
				print(text)
				#anotations_list = list(set(anotations_list))
				...
				#print(full_list)
print(full_list)
with open("all_annotations.json", "w") as outfile:
